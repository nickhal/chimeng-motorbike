  ...
  return (
    ...
    >
      <div className="relative h-[250px] overflow-hidden">
        ...
        />
        <div
          className={`absolute inset-0 bg-gradient-to-t from-brand-black/80 to-transparent transition-opacity duration-500 ${isHovered ? "opacity-100" : "opacity-70"}`}
        >
          <div className="absolute bottom-0 left-0 p-6">
            <h3 className="font-sans text-2xl font-bold text-white uppercase">{name}</h3>
            {isNew && (
      ...
      <div className="p-6">
        ...
        <div className="flex justify-between items-center">
          ...
          <Button className="bg-brand-red hover:bg-brand-red/90 text-white">Book at {name}</Button>
        </div>
      </div>
    </div>
  )
